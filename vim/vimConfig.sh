...
DEPS="git git-core build-essential cmake python-dev vim-gnome cscope automake pkg-config libpcre3-dev zlib1g-dev liblzma-dev clang-3.9 clang-format-3.9"
...
mkdir ~/.vim/UltiSnips
for f in `find ~/.myconfig/vim/UltiSnips/*.snippets | xargs -n 1 basename`
do
...
#Create symlink from clang-format-3.9 to clang-format
sudo ln -s ../lib/llvm-3.9/bin/clang-format /usr/bin/clang-format
